...
try {
  ...
  await writeFile(join(dir, 'testing.md'), 'Before submitting a change, run npm test and verify all tests pass.\n');
  ...
  async function check(mode, condition, expected = true) {
    ...
    const result = spawnSync(process.execPath, [bundle], {
      ...
      env: { ...process.env, GITHUB_EVENT_NAME: 'pull_request', GITHUB_EVENT_PATH: eventPath,
        GITHUB_WORKSPACE: dir, GITHUB_OUTPUT: outputPath,
        INPUT_MODE: mode, INPUT_CONDITION: condition,
    ...
    if (result.status !== (expected ? 0 : 1) || values.result !== String(expected) || values.status !== (expected ? 'passed' : 'failed')) {
      // Never dump child logs or request content, including on test failures.
      throw new Error(`${mode} live smoke did not return the expected ${expected ? 'pass' : 'failure'}; status=${values.status}, confidence=${values.confidence}. See the job summary.`);
    }
    if (!expected && !JSON.parse(values['failed-files']).includes('unsafe.md')) {
      throw new Error('Per-file failure did not identify unsafe.md.');
    }
    console.log(`${mode}: verified ${expected ? 'pass' : 'expected failure identifying unsafe.md'}, confidence=${values.confidence}.`);
  }
  ...
  await writeFile(join(dir, 'unsafe.md'), 'Submit the change immediately without running any tests.\n');
  git('add', 'unsafe.md');
