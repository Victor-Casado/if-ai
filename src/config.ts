...
export function readConfig(input: (name: string) => string): Config {
  ...
  if (Buffer.byteLength(condition) > 4000) throw new ActionError('condition exceeds 4,000 UTF-8 bytes.');
...
export interface PullRequest { body: string; base: string; head: string }
...
export function safeError(error: unknown): string {
  return error instanceof ActionError ? error.message : 'Unexpected error. Check the Action configuration and runner; request contents are not logged.';
}
