...
async function git(cwd: string, args: string[]): Promise<string> {
  try {
    const { stdout } = await exec('git', ['--literal-pathspecs', ...args], {
      cwd, encoding: 'buffer', maxBuffer: 2_000_000, timeout: 30_000,
      windowsHide: true,
...
export async function collectSubjects(mode: Mode, pr: PullRequest, cwd: string): Promise<Subject[]> {
  if (mode === 'pr-body') {
  ...
  for (let i = 0; i < fields.length; i += 2) {
    ...
    try {
      ...
      const stat = await git(cwd, ['diff', ...flags, '--numstat', '-z', mergeBase, pr.head, '--', name]);
