...
import { mkdtemp, writeFile, rm, rename, unlink } from 'node:fs/promises';
...
it('reads PR body without needing a Git checkout', async () => {
  expect(await collectSubjects('pr-body', { body: 'Expected behavior and tests', base: '', head: '' }, '/missing')).toEqual([
    { name: 'PR body', content: 'Expected behavior and tests' },
  ]);
  await expect(collectSubjects('pr-body', { body: ' ', base: '', head: '' }, '/missing')).rejects.toThrow('empty');
});
...
it('identifies binary files and LFS pointers instead of judging incomplete content', async () => {
  ...
  await writeFile(join(cwd, 'large.dat'), 'version https://git-lfs.github.com/spec/v1\noid sha256:123\nsize 100\n');
