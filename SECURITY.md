...
Public PRs can trigger paid API calls. Before enabling either `pull_request_target` example, create the `jev-approved` environment in repository settings, add a required maintainer reviewer, and store `OPENROUTER_API_KEY` as an environment secret. For direct TypeSafe access, set `provider: typesafe` and reference `TYPESAFE_API_KEY` instead. Declaring an environment name in YAML does not add reviewers automatically. Approve the exact run you intend to fund. Repository-level approval for external contributors does not protect `pull_request_target` runs.
